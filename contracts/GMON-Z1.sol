...
contract GMON_Z1 is IERC20, Ownable {

    string private constant _name = "Game Z1 Planet";
    string private constant _symbol = "GMON-Z1";
    ...
    modifier launchRestrict(address sender, address recipient, uint256 amount) {
        ...
        if (state == State.Restricted) {
            ...
            require(lastTx[sender].add(60) <= block.timestamp && lastTx[recipient].add(60) <= block.timestamp, "GMON: only one tx/min in restricted mode");
