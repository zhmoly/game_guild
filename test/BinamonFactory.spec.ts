...
import { privateKey, BMON, BNRG, BMON_Z1, BMONC, BMONEC, BMONC_Z1, BLANDC } from '../settings.json'
...
describe('BinamonFactory', () => {
  let wallet: Wallet, other: Wallet

  let factory: BinamonFactory
  let poolBytecode: string
  const fixture = async () => {
    const factoryContract = await ethers.getContractFactory('BinamonFactory')
    return (await factoryContract.deploy(BMON, BNRG, BMON_Z1, BMONC, BMONEC, BMONC_Z1, BLANDC)) as BinamonFactory
  }

  let loadFixture: ReturnType<typeof createFixtureLoader>
  before('create fixture loader', async () => {
    [wallet, other] = await (ethers as any).getSigners()
    console.log(wallet, other)

    loadFixture = createFixtureLoader([wallet, other])
  })

  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('BinamonFactory')).bytecode
  })
